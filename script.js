...
function updateValues() {
  array_container_width = Math.floor($("#main").width());
...
function disableOthers() {
  ...
  $("#newc").prop("disabled", true);
  $("#size-slide").prop("disabled", true);
}

function enableOthers() {
  $("#sort").prop("disabled", false);
  $("#newc").prop("disabled", false);
  $("#size-slide").prop("disabled", false);
}
...
$(document).ready(function () {
  $("#size-slide").attr("min", MIN_SIZE);
  $("#size-slide").attr("max", MAX_SIZE);
  $("#size-slide").attr("value", DEFAULT_SIZE);

  $("#speed-slide").attr("min", MIN_SPEED);
  $("#speed-slide").attr("max", MAX_SPEED);
  $("#speed-slide").attr("value", DEFAULT_SPEED);
  ...
  $("#sort").click(async function () {
    ...
    else if (algo_selected == "Quick Sort") await quicksort(0, size - 1);
    else if (algo_selected == "Heap Sort") await heapsort();
    else {
  ...
  $("#speed-slide").on("input", function () {
    delay = WAITING_TIME * Math.pow(2, MAX_SPEED - $(this).val());
  });

  $(window).resize(function () {
    if (array_container_width != Math.floor($("#main").width())) {
      updateValues();
